...
for yy=5%:numel(data)
    ...
    u = delta_omega_ctr; y = imu_raw_gyro_x(1:length(u));
...
subplot(1,2,2)
    ...
    subplot(1,2,1)
for ii=1:numel(data),tmp = cell2mat(data{ii}.fdt.P)'; if numel(tmp)==2,  poli(ii,:) = cell2mat(data{ii}.fdt.P)'; else poli(ii,:)=[0,0]; end ,end
