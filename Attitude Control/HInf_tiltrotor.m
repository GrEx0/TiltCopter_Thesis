...
sys_delay = exp(-0.09006*s);

% Padè approssimation of the time delay
sys_delay = pade(sys_delay,1);
...
% Outer regulator
Cphi0 = tunablePID('Cphi','pd');                
Cphi0.Kp.Value = KRP;               % initialize Kp
Cphi0.Kd.Value = KRD;               % initialize Kd
...
% Innter regulator
Cp0 = tunablePID('Cp','pid');
Cp0.Kp.Value = Kpp;
Cp0.Ki.Value = Kip;
Cp0.Kd.Value = Kdp;
Cp0.Tf.Value = 0.01;                % set parameter Tf
Cp0.Tf.Free = false;                % fix parameter Tf to this value
...
InnerLoop0 = feedback(motor*X3*G_p*X2*sys_delay*mixer*X1*Cp0,1);
InnerLoop0.u = 'p_ref';
...
CL0.inputName = 'phi_0'; CL0.OutputName = 'phi';


%% TUNING REQUIREMENTS
% wc = 9;                                       % bandwidth of the identified system [rad/s]
% response_time = 2/wc;                         % [s]
% steady_state_error = 0.01;
% Rtrack = TuningGoal.Tracking('phi_0','phi',response_time,steady_state_error);
% Rreject = TuningGoal.Gain('p','phi',0.1);   % Disturbance injected at p is suppressed at phi by a factor of 10
...
attfact = frd([100 1 1],[0.001*wc wc 100*wc]);
