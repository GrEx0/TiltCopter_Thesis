...
%     3. Define and select and  a model structure (a set of candidate system
...
%   added the output by specifying the signal-to-noise ratio (SNR).
...
%   A data object is then created that will include both output and input
...
%   select the first 1000 data for the model identification.
...
%   To choose a model structure that is suitable to represent the system 
...
%   orders and delays.
%   A model order selection algorithm associated to the SRIVC model estimation
%   method allows the user to automatically search over a whole range of
%   different model orders. 
%   Two statistical measures are then used for the analysis.
...
%   The best model structures sorted according to YIC are displayed with 
    selcstruc(V);
%   Press a key to continue
    
%   The best trade-off for both criteria RT2 (closer to 1) and YIC (the  
%   most negative) is here obtained for nn=[2 4 0]
%   which is the true model structure of the considered system.
%   Press a key to continue
    pause
    close all
    clc
...
%   This estimator is really reliable and it is recommended to use it as a 
%   first choice in practice.
%
%   The extra information needed for the routine is:
%     - the number of numerator and denominator parameters and number of 
%       samples for the delay of the model [nb nf nk] stored in the 
%       variable nn that has previously been selected
%       
...
%   input signal. We can thus be satisfied with the identified COE 1/4 model. 
%
%   Press any key
    pause    
    clc
